...
public class APIAfterServerAbortReceivePublicationTest {
	
	/** The Constant testLogger. */
	protected static final Logger testLogger = Logger.getLogger(Loggers.TEST.getValue());
	/** The Constant logger. */
	protected final static Logger logger = Logger.getLogger(APIAfterServerAbortReceivePublicationTest.class);
	...
	private class MsgCallback extends SCMessageCallback {
		...
		public MsgCallback(SCService service) {
			super(service);
			APIAfterServerAbortReceivePublicationTest.messageReceived = false;
			response = null;
			messageCounter = 0;
			expectedMessages = 0;
		}
		...
		public void receive(SCMessage msg) {
			...
			if ( expectedMessages == messageCounter) {
				APIAfterServerAbortReceivePublicationTest.messageReceived = true;
			}
			if (((messageCounter+1) % 1000) == 0) {
				APIAfterServerAbortReceivePublicationTest.testLogger.info("Receiving message nr. " + (messageCounter+1));
			}
		}
