...
public class APIAttachDetachClientTest extends APIIntegrationSuperClientTest {
	...
	/**
	 * Description: setKeepAliveTimeoutSeconds after attach.<br>
	 * Expectation: throws SCServiceException
	 */
	@Test(expected = SCServiceException.class)
	public void t131_setKeepAliveTimeoutSeconds() throws Exception {
		client = newSCClient(TestConstants.HOST, TestConstants.PORT_SC0_TCP, ConnectionType.NETTY_TCP);
		client.attach();
		Assert.assertEquals("Client is not attached", true, client.isAttached());
		client.setKeepAliveTimeoutSeconds(10); // too late => throws SCServiceException
	}
