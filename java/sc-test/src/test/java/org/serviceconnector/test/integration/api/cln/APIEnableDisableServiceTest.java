...
public class APIEnableDisableServiceTest extends APIIntegrationSuperClientTest {
	...
	public void t05_checkDefaultState() throws Exception {
		client = new SCMgmtClient(TestConstants.HOST, TestConstants.PORT_SC_TCP, ConnectionType.NETTY_TCP);
		client.attach();
		Assert.assertEquals("Enabled ", Boolean.TRUE.toString(), client.isServiceEnabled(TestConstants.sesServiceName1)
	...
	public void t07_disableEnable() throws Exception {
		...
		Assert.assertEquals("Enabled ", Boolean.TRUE.toString(), client.isServiceEnabled(TestConstants.sesServiceName1)
				.getParamValue(TestConstants.sesServiceName1));
		client.disableService(TestConstants.sesServiceName1);
		Assert.assertEquals("Disabled ", Boolean.FALSE.toString(), client.isServiceEnabled(TestConstants.sesServiceName1)
				.getParamValue(TestConstants.sesServiceName1));
