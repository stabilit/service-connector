...
public class DemoPublishClient extends Thread {
	...
	public void run() {
		...
		try {
			...
			service.subscribe(cbk, mask, 10, "subscription-info");	//alternative with operation timeout and subscr info
			...
			try {
				...
				service.unsubscribe(10, "session-info");			// alternative with operation timeout and session info
				sc.detach();
			} catch (Exception e) {
