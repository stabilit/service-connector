...
public final class SCCacheFactory {
	 ...
	 *
	 * maxElementsInMemory: Sets the maximum number of objects that will be created in memory. 0 = no limit. In practice no limit means Integer.MAX_SIZE (2147483647) unless the
	 * cache is distributed with a Terracotta server in which case it is limited by resources.
	 *
	 ...
	 *
	 * The following attributes and elements are optional. overflowToOffHeap: (boolean) This feature is available only in enterprise versions of Ehcache. When set to true, enables
	 * the cache to utilize off-heap memory storage to improve performance. Off-heap memory is not subject to Java GC. The default value is false.
	 *
	 * maxMemoryOffHeap: (string) This feature is available only in enterprise versions of Ehcache. Sets the amount of off-heap memory available to the cache. This attribute's
	 * values are given as <number>k|K|m|M|g|G|t|T for kilobytes (k|K), megabytes (m|M), gigabytes (g|G), or terabytes (t|T). For example, maxMemoryOffHeap="2g" allots 2 gigabytes
	 * to off-heap memory. This setting is in effect only if overflowToOffHeap is true. Note that it is recommended to set maxElementsInMemory to at least 100 elements when using
	 * an off-heap store, otherwise performance will be seriously degraded, and a warning will be logged. The minimum amount that can be allocated is 128MB. There is no maximum.
	 *
	 * timeToIdleSeconds: Sets the time to idle for an element before it expires. i.e. The maximum amount of time between accesses before an element expires Is only used if the
	 * element is not eternal. A value of 0 means that an Element can idle for infinity. The default value is 0.
	 *
	 * timeToLiveSeconds: Sets the time to live for an element before it expires. i.e. The maximum time between creation time and when an element expires. Is only used if the
	 * element is not eternal. A value of 0 means that and Element can live for infinity. The default value is 0.
	 *
	 ...
	 *
	 * diskSpoolBufferSizeMB: This is the size to allocate the DiskStore for a spool buffer. Writes are made to this area and then asynchronously written to disk. The default size
	 * is 30MB. Each spool buffer is used only by its cache. If you get OutOfMemory errors consider lowering this value. To improve DiskStore performance consider increasing it.
	 ...
	 *
	 * statistics: Whether to collect statistics. Note that this should be turned on if you are using the Ehcache Monitor. By default statistics is turned off to favour raw
	 * performance. To enable set statistics="true"
	 *
	 * memoryStoreEvictionPolicy: Policy would be enforced upon reaching the maxElementsInMemory limit. Default policy is Least Recently Used (specified as LRU). Other policies
	 ...
	 *
	 * @param scCacheConfiguration the SC cache configuration
	 * @param cacheType the cache type needed
