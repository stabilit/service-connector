...
public class RemoteNodeConfiguration {
	 ...
	 * The Constructor.
	 *
	 * @param name the remote node name
	 */
	public RemoteNodeConfiguration(String name) {
		this(ServerType.UNDEFINED, name, null, 0, null, Constants.DEFAULT_MAX_CONNECTION_POOL_SIZE, Constants.DEFAULT_KEEP_ALIVE_INTERVAL_SECONDS,
				Constants.DEFAULT_CHECK_REGISTRATION_INTERVAL_SECONDS, 0, Constants.SLASH);
	}
	 ...
	 * @param keepAliveIntervalSeconds the keep alive interval seconds
	 * @param maxConnections the max connections
	 * @param maxSessions the max sessions
	 */
	public RemoteNodeConfiguration(ServerType serverType, String name, String host, int port, String connectionType, int keepAliveIntervalSeconds,
			int checkRegistrationIntervalSeconds, int maxConnections, int maxSessions, String httpUrlFileQualifier) {
		this.name = name;
		this.host = host;
		this.port = port;
	 ...
	 * @param maxConnections the max connections
	 */
	public RemoteNodeConfiguration(String name, String host, int port, String connectionType, int keepAliveIntervalSeconds, int checkRegistrationIntervalSeconds,
			int maxConnections) {
		this(ServerType.UNDEFINED, name, host, port, connectionType, keepAliveIntervalSeconds, checkRegistrationIntervalSeconds, maxConnections, 0, Constants.SLASH);
	}
	...
	public void load(CompositeConfiguration compositeConfig) throws SCMPValidatorException {
		...
		if ((serverType == ServerType.FILE_SERVER) || (serverType == ServerType.WEB_SERVER) || (serverType == ServerType.CASCADED_SC)) {
			// get max connection pool size
			Integer localMaxPoolSize = compositeConfig.getInteger(this.name + Constants.PROPERTY_QALIFIER_MAX_CONNECTION_POOL_SIZE, null);
			...
			Integer localKeepAliveIntervalSeconds = compositeConfig.getInteger(this.name + Constants.PROPERTY_QUALIFIER_KEEP_ALIVE_INTERVAL_SECONDS, null);
