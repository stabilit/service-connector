 ...
 * <p>
 * Using this class is a simpler alternative to using a PipedInputStream and a PipedOutputStream. PipedInputStreams and PipedOutputStreams don't support the mark operation, don't
 * allow you to control buffer sizes that they use, and have a more complicated API that requires instantiating two classes and connecting them.
 * <p>
...
public class CircularByteBuffer {
	 ...
	 * The circular buffer.
	 * <p>
	 * The actual capacity of the buffer is one less than the actual length of the buffer so that an empty and a full buffer can be distinguished. An empty buffer will have the
	 * markPostion and the writePosition equal to each other. A full buffer will have the writePosition one less than the markPostion.
	 * <p>
	 * There are three important indexes into the buffer: The readPosition, the writePosition, and the markPosition. If the InputStream has never been marked, the readPosition and
	 * the markPosition should always be the same. The bytes available to be read go from the readPosition to the writePosition, wrapping around the end of the buffer. The space
	 * available for writing goes from the write position to one less than the markPosition, wrapping around the end of the buffer. The bytes that have been saved to support a
	 * reset() of the InputStream go from markPosition to readPosition, wrapping around the end of the buffer.
	 *
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * <p>
	 * Write methods may throw a BufferOverflowException if the buffer is not large enough. A large enough buffer size must be chosen so that this does not happen or the caller
	 * must be prepared to catch the exception and try again once part of the buffer has been consumed.
	 *
	 *
	...
	protected class CircularByteBufferInputStream extends InputStream {
		...
		/**
		 * Returns the number of bytes that can be read (or skipped over) from this input stream without blocking by the next caller of a method for this input stream. The next
		 * caller might be the same thread or or another thread.
		 *
		...
		/**
		 * Close the stream. Once a stream has been closed, further read(), available(), mark(), or reset() invocations will throw an IOException. Closing a previously-closed
		 ...
		 *
		 * @param readAheadLimit Limit on the number of bytes that may be read while still preserving the mark. After reading this many bytes, attempting to reset the stream will
		 *        fail.
		 *
		 * @since ostermillerutils 1.00.00
		 */
		...
		/**
		 * Reset the stream. If the stream has been marked, then attempt to reposition i at the mark. If the stream has not been marked, or more bytes than the readAheadLimit have
		 * been read, this method has no effect.
		 *
		 * @throws IOException if the stream is closed.
		 *
		 * @since ostermillerutils 1.00.00
	...
	protected class CircularByteBufferOutputStream extends OutputStream {
		...
		/**
		 * Close the stream, flushing it first. This will cause the InputStream associated with this circular buffer to read its last bytes once it empties the buffer. Once a
		 * stream has been closed, further write() or flush() invocations will cause an IOException to be thrown. Closing a previously-closed stream, however, has no effect.
		 *
		 * @throws IOException never.
		 *
		 * @since ostermillerutils 1.00.00
		 ...
		 * @throws BufferOverflowException if buffer does not allow blocking writes and the buffer is full. If the exception is thrown, no data will have been written since the
		...
		/**
		 * Write a single byte. The byte to be written is contained in the 8 low-order bits of the given integer value; the 24 high-order bits are ignored. If the buffer allows
		 * blocking writes, this method will block until all the data has been written rather than throw an IOException.
		 *
		 * @param c number of bytes to be written and the buffer is full.
		 * @throws IOException if the stream is closed, or the write is interrupted.
		 *
		 * @since ostermillerutils 1.00.00
		 */
