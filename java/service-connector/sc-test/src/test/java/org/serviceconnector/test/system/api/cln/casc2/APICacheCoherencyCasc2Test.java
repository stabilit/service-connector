...
public class APICacheCoherencyCasc2Test extends APICacheCoherencyCasc1Test {
	...
	public APICacheCoherencyCasc2Test() {
		APISystemSuperCCTest.setUp2CascadedServiceConnectorAndServer();
	}

	/**
	 * Description: 01: disconnect client to top level (cascaded) SC 02: connect new client to SC0 03: load data to cache (cid=700) on SC0 04: start cache guardian - publish 3
	 * large Appendix 05: verify callback retrieval - 3 appendix within 10sec 06: verify data is in cache on SC0 07: start cache guardian over top level (cascaded) SC - doing
	 * nothing 08: stop cache guardian over SC0 - cached managed data should stay 09: verify data is still in cache on SC0 10: verify data is NOT in top level cache 11: reconnect
	 * client to top level (cascaded) SC - read data from cache 12: verify data is correct 13: verify data is now in top level cache Expectation: passes
	 */
	@Test
	public void t40_cc_ManagedDataToSC0PublishAppendixClientToCascSC() throws Exception {
		...
		this.checkCacheInspectString(inspectResponse, "success", SC_CACHE_ENTRY_STATE.LOADED, "700", "3", "700/0/0=0&700/1/0=1&700/2/0=1&700/3/0=1&", "unset");
	}

	/**
	 * Description: 01: connect new client2 to top level (cascaded) SC 02: load data to cache (cid=700) by client1 03: start cache guardian1 of client1 - do nothing 04: start cache
	 * guardian1A of client2 - publish 3 large appendix 05: verify callback of client2 guardian1A retrieval - 3 appendix within 10sec 06: verify callback of client1 guardian1
	 * retrieval - 0 appendix within 3sec 07: verify data is now in top level cache Expectation: passes
	 */
	@Test
	...
	/**
	 * Description: 01: connect new client2 to top level (cascaded) SC 02: load data to cache (cid=700) by client1 03: start cache guardian1 of client1 - publish 1 large initial
	 * 04: verify callback of client1 guardian1 retrieval - 1 large initial message within 10sec 05: start cache guardian1A of client2 - publish 1 large initial 06: verify callback
	 * of client2 guardian1A retrieval - 1 large initial message within 10sec 06: verify callback of client1 guardian1 retrieval - 0 large initial message within 2sec 08: verify
	 * data is now in top level cache Expectation: passes
	 */
	@Test
