...
import com.stabilit.sc.common.net.nio.tcp.ChangeRequest;

public class NioClient extends ClientConnectionAdapter implements Runnable {
	...
	private void write(SelectionKey key) throws IOException {
		...
		synchronized (this.pendingData) {
			...
			if(queue == null) //TODO added by joël
				return;
			while (!queue.isEmpty()) {
	...
	public SCMP sendAndReceive(SCMP scmp) throws Exception {
		...
		RspHandler handler = new RspHandler();
		send(data, handler);
		byte[] result = handler.waitForResponse();
		ByteArrayInputStream bais = new ByteArrayInputStream(result);
		encoderDecoder = EncoderDecoderFactory.newInstance(result);
