...
public class ServerSPListener extends ClientListener {
	...
	public void messageReceived(IPoolConnection conn, SCMP scmp) throws Exception {
		...
		if (scmp.getMessageId().equals("register"))
			log.debug("Messages register confirmed " + scmp.getMessageId() + " on TcpSPServerListener");

		if (scmp.getMessageId().equals("publish") && count < 15) {
			log.debug("Messages publish confirmed " + scmp.getMessageId() + " on TcpSPServerListener");
			log.debug("next publish msg sent. " + count);
