 ...
 * 
 */
public class SCKernelStarter implements SCKernelConstants, Runnable{
	...
	public static void main(String[] args) throws SCKernelException {
		SCKernelStarter.startHTTPSCServer();
		SCKernelStarter.startTCPSCServer();
	}
	...
	public static Properties loadProperties(String fileName) throws SCKernelException {
		...
		try {
			...
			try {
				props.load(SCKernelStarter.class.getResourceAsStream(fileName));
			} catch (IOException e) {
				e.printStackTrace();
			}
		} catch (IOException e) {
	...
	public static void startHTTPSCServer() throws SCKernelException {
		props = SCKernelStarter.loadProperties(HTTP_CONFIG_FILE);
	...
	public static void startTCPSCServer() throws SCKernelException {
		props = SCKernelStarter.loadProperties(TCP_CONFIG_FILE);
		Server server = new Server();
		server.setProps(props);
		Thread thread = new Thread(server);
		thread.start();
