            ...
            <tr class="sc_table_even">
              <xsl:for-each select="$body/maintenance/services/service">
                ...
                <xsl:if test="scDownloadService = 'true'">
                   <td id="sc_file_service_download_{name}" style="float:left"><input class="sc_form_button" style="margin:10px;" name="File Service" type="button" value="Property File Download - {name}" onclick="javascript:maintenanceCall('sc_property_download', '{name}')"></input></td>
                </xsl:if>                
