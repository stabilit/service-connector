...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:variable name="subscriptions" select="$body/subscriptions"/>    
    <xsl:template name="sc_script">
      setInterval('infoCall()', 5000);	    
      setInterval("contentCall('<xsl:value-of select="$urlencoded"/>', 'subscriptions', 'server=<xsl:value-of select="$serverParam"/>&amp;service=<xsl:value-of select="$serviceParam"/>&amp;page=<xsl:value-of select="$page"/>&amp;site=<xsl:value-of select="$site"/>&amp;sim=<xsl:value-of select="$sim"/>')", 10000);            
    </xsl:template>
      ...
      <div class="sc_table max_width">
        <div class="sc_table_title">
           <xsl:call-template name="pageArea">
             ...
             <xsl:with-param name="lastPage" select="$subscriptions/@lastPage"/>
             <xsl:with-param name="lastSite" select="$subscriptions/@lastSite"/>
             <xsl:with-param name="siteSize" select="$subscriptions/@siteSize"/>
             <xsl:with-param name="pageSize" select="$subscriptions/@pageSize"/>
           </xsl:call-template>
        </div>             
	  ...
	  <div class="sc_table_details">
        ...
        <table border="0" class="sc_table" cellspacing="0" cellpadding="0">
          <tr class="sc_table_header">          
            ...
            <th class="sc_table">Subscription Count</th>
