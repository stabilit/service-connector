 ...
 * PipedOutputStreams don't support the mark operation, don't allow you to control buffer sizes that they use, and have a more
...
public class CircularByteBuffer {
	 ...
	 * The circular buffer.
	 * <p>
	 * The actual capacity of the buffer is one less than the actual length of the buffer so that an empty and a full buffer can be
	 * distinguished. An empty buffer will have the markPostion and the writePosition equal to each other. A full buffer will have
	 * the writePosition one less than the markPostion.
	 * <p>
	 * There are three important indexes into the buffer: The readPosition, the writePosition, and the markPosition. If the
	 * InputStream has never been marked, the readPosition and the markPosition should always be the same. The bytes available to be
	 * read go from the readPosition to the writePosition, wrapping around the end of the buffer. The space available for writing
	 * goes from the write position to one less than the markPosition, wrapping around the end of the buffer. The bytes that have
	 * been saved to support a reset() of the InputStream go from markPosition to readPosition, wrapping around the end of the
	 * buffer.
	 * 
	 * @since ostermillerutils 1.00.00
	 */
	 ...
	 * this buffer.
	 * <p>
	 * Write methods may throw a BufferOverflowException if the buffer is not large enough. A large enough buffer size must be
	 * chosen so that this does not happen or the caller must be prepared to catch the exception and try again once part of the
	 ...
	 * <p>
	 * Note that the number of bytes available plus the number of bytes free may not add up to the capacity of this buffer, as the
	 * buffer may reserve some space for other purposes.
	 * 
	 ...
	 * is available rather than throw an exception.
	 * <p>
	 * Note that the buffer may reserve some bytes for special purposes and capacity number of bytes may not be able to be written
	 * to the buffer.
	 * <p>
